...
def read_config():
    global HOST, USER_NAME, PASS_WORD, DIRS, SUFFIX, ENCODING, TEMP_DIR, TIMEOUT
    # 当前文件路径
    ...
    TIMEOUT = int(conf.get('HOSTCONFIG', 'timeout'))
...
def connect_ftp():
    ...
    try:
        ...
        m_text.insert(tk.END, cur_ftp.getwelcome()+'用户名  '+USER_NAME+'   连接FTPd服务器成功 !  \n')
        m_text.pack()
        m_text.update()
        return cur_ftp
    except(socket.error, socket.gaierror):  # ftp 连接错误
...
def check_samename(l_name, r_name, c_ftp):
    ...
    for fn1 in ftp_name:
        if l_name == fn1.split('.')[0]:
            ...
            alarm(alarm1)
            r1 = tkinter.messagebox.askokcancel(title='askokcancel', message='是否删除目标文件后重传？')
            # print(r1, fn1)
            if r1:
                c_ftp.delete(fn1)
                return True
